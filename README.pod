...
fragmented (dpkg, aptitude, apt-get, apt-cache, apt-file...). Just one simple
extensible tool combining the most frequently used functions of the mentioned
tools. The available functions are fully customizable - One can even be adjusted
to a completely different package manager.
